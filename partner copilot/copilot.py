...
class RAGRetriever:
    ...
    def _build_index(self):
        ...
        linkedin_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "scraped_linkedin_data.json"
        )
        scraped_jobs = []
        if os.path.exists(linkedin_file):
            try:
                ...
                logger.info(
                    f"Loaded {len(scraped_jobs)} real-time scraped jobs from {linkedin_file}"
                )
            except Exception as e:
                logger.error(f"Error loading scraped JSON file {linkedin_file}: {e}")
        else:
            logger.warning(
                f"Scraped data file not found at {linkedin_file}. Please run the web scraper to populate it."
            )
        ...
        for doc in self.documents:
            ...
            for word in query_words:
                ...
                if word in doc_text_lower:
                    score += 1
                    if (
                        doc["type"] == "linkedin_job" and word in doc["text"].split("\n")[2].lower()
                    ):  # Title line
                        score += 2
                    if (
                        doc["type"] == "user_guide" and word in doc["text"].split("\n")[0].lower()
                    ):  # Section header line
                        score += 2
...
def get_embeddings_in_batches(
    client, texts: List[str], batch_size: int = 8, model: str = "text-embedding-3-small"
) -> List[List[float]]:
    """
...
class SignalCopilot:
    ...
    def _call_completions_with_retry(
    ...
    def _offline_fallback(
        ...
        if not retrieved:
            answer = (
                "I cannot find this information in the scraped LinkedIn database or user guide."
            )
        else:
